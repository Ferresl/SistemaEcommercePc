...
public class CarritoService {
    ...
    public CarritoResponseDTO buscarPorId(Integer id) { 
        return toResponse(obtenerCarrito(id)); 
    }
    
    public CarritoResponseDTO buscarPorUsuario(Integer usuarioId) { 
        return carritoRepository.findByUsuarioId(usuarioId).map(this::toResponse).orElseThrow(() -> new ResourceNotFoundException("Carrito no encontrado")); 
    }
    
    public CarritoResponseDTO crear(CarritoCreateDTO dto) { 
    ...
    public CarritoResponseDTO actualizar(Integer id, CarritoUpdateDTO dto) { 
        usuarioClient.obtenerPorId(dto.getUsuarioId()); 
    ...
    public CarritoResponseDTO agregarItem(Integer carritoId, AgregarItemCarritoDTO dto) { 
        Carrito carrito = obtenerCarrito(carritoId); 
        ProductoResponseDTO producto = productoClient.obtenerPorId(dto.getProductoId()); 
        validarStock(dto.getProductoId(), dto.getCantidad()); 
        ...
        .configuracionId(dto.getConfiguracionId())
        .cantidad(dto.getCantidad())
        .precioUnitario(producto.getPrecio())
        .subtotal(subtotal).build(); 
        itemRepository.save(item); 
        recalcular(carrito.getId()); 
        return buscarPorId(carrito.getId()); 
    }
    
    public CarritoResponseDTO actualizarCantidad(Integer itemId, ActualizarCantidadItemDTO dto) { 
        ItemCarrito item = obtenerItem(itemId); 
        validarStock(item.getProductoId(), 
        dto.getCantidad()); 
        item.setCantidad(dto.getCantidad()); 
        item.setSubtotal(item.getPrecioUnitario().multiply(BigDecimal.valueOf(dto.getCantidad()))); 
        itemRepository.save(item); 
        recalcular(item.getCarritoId()); 
        return buscarPorId(item.getCarritoId()); 
    }
    
    public void eliminarItem(Integer itemId) { 
        ItemCarrito item = obtenerItem(itemId); 
        Integer carritoId = item.getCarritoId(); 
        itemRepository.delete(item); 
        recalcular(carritoId); 
    }
    
    public void vaciar(Integer carritoId) { 
        List<ItemCarrito> items = itemRepository.findByCarritoId(carritoId); 
        itemRepository.deleteAll(items); 
        recalcular(carritoId); 
    }
    
    private void validarStock(Integer productoId, Integer cantidad) { 
        InventarioResponseDTO inv = inventarioClient.obtenerPorProducto(productoId); 
        if (inv.getStockDisponible() < cantidad) { throw new BadRequestException("Stock insuficiente"); } 
    }
    
    private void recalcular(Integer carritoId) { 
        Carrito carrito = obtenerCarrito(carritoId); 
        BigDecimal total = itemRepository.findByCarritoId(carritoId).stream().map(ItemCarrito::getSubtotal).reduce(BigDecimal.ZERO, BigDecimal::add); 
        carrito.setSubtotal(total); 
        carrito.setTotal(total); 
        carritoRepository.save(carrito); 
    }
    
    private Carrito obtenerCarrito(Integer id) { 
        return carritoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Carrito no encontrado con id " + id)); 
    }
    
    private ItemCarrito obtenerItem(Integer id) { 
        return itemRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Item no encontrado con id " + id)); 
    }
    
    private ItemCarritoResponseDTO itemResponse(ItemCarrito item) { 
        return ItemCarritoResponseDTO.builder()
        .id(item.getId())
        .carritoId(item.getCarritoId())
        .productoId(item.getProductoId())
        .configuracionId(item.getConfiguracionId())
    ...
    private CarritoResponseDTO toResponse(Carrito carrito) { 
        ...
        .items(itemRepository.findByCarritoId(carrito.getId()).stream().map(this::itemResponse).toList()).build(); 
    }
}
