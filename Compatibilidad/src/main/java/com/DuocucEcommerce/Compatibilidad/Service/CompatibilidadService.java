...
public class CompatibilidadService {
    ...
    public CompatibilidadResponseDTO evaluar(CompatibilidadRequestDTO dto) {
        ...
        if (!cpu.getSocket().equalsIgnoreCase(placa.getSocket())) { 
            motivos.add("CPU y placa madre no tienen el mismo socket"); }
        
        if (!ram.getTipoMemoria().equalsIgnoreCase(placa.getTipoRamSoportada())) { 
            motivos.add("La RAM no es compatible con la placa madre"); }
        
        if (gpu.getLargoMm() > gabinete.getLargoMaxGpuMm()) { 
            motivos.add("La GPU no cabe en el gabinete"); }
        
        if (fuente.getPotenciaWatts() < dto.getTdpTotal() + 100) { 
            motivos.add("La fuente no tiene potencia suficiente"); }
        
        if (!gabinete.getFormatoSoportado().toLowerCase().contains(placa.getFormato().toLowerCase())) { 
            motivos.add("El gabinete no soporta el formato de la placa madre"); 
        }
    ...
    public CompatibilidadResponseDTO buscarPorConfiguracion(Integer configuracionId) { 
        return resultadoRepository.findTopByConfiguracionIdOrderByIdDesc(configuracionId).map(this::toResponse).orElseThrow(() -> new ResourceNotFoundException("Resultado no encontrado")); 
    }
    
    public CompatibilidadResponseDTO buscarPorId(Integer id) { 
        return resultadoRepository.findById(id).map(this::toResponse).orElseThrow(() -> new ResourceNotFoundException("Resultado no encontrado")); 
    }
    ...
    private CompatibilidadResponseDTO toResponse(ResultadoCompatibilidad r) {
        ...
        return CompatibilidadResponseDTO.builder()
        .resultadoId(r.getId())
        .configuracionId(r.getConfiguracionId())
        .estado(r.getEstado())
