...
public class NotificacionService {
    ...
    public NotificacionResponseDTO actualizar(Integer id, NotificacionUpdateDTO dto) { 
    ...
    private Notificacion obtener(Integer id) { 
        return repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Notificacion no encontrada con id " + id)); 
    }

    private void copiar(NotificacionCreateDTO dto, Notificacion n) { 
        n.setUsuarioId(dto.getUsuarioId()); 
        n.setTitulo(dto.getTitulo()); 
        n.setMensaje(dto.getMensaje()); 
        n.setTipo(dto.getTipo()); 
